            ...
            <div class="animated fadeIn">
                <div class="row">
                            ...
                            <div class="card-body">
                                ...
                                <tr>
                                 <th scope="row">{{ $index +$names->firstitem()}}</th>
                                    <td><img src="{{ asset('storage/posts/'.$row->thumbnail) }}" class="rounded" style="width: 150px"></td>
                                    <td>{{ $row->nama}}</td>
                                    <td>{{ $row->merk}}</td>
                                    <td>{{ $row->type}}</td>
                    ...
                    @endif
                        ...
                        <td> <a href="/deletebarang/{{ $row->id }}" class="btn btn-sm btn-danger mb-2">Hapus</a>
                        <a href="/editbarang/{{ $row->id }}" class="btn btn-sm btn-warning mb-2">Edit</a>
                            <button type="button" class="btn btn-sm btn-primary mb-2" data-toggle="modal" data-target="#qrCodeModal-{{ $row->id }}">Show QR Code <i class="fa fa-qrcode"></i></button>
        <a href="{{ route('scan.qr.code', ['id' => $row->id]) }}" class="btn btn-sm btn-secondary">Scan QR Code <i class="fa fa-qrcode"></i></a>
                        </td>
